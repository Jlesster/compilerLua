...
function M.action(selected_option)
  ...
  local files = utils.find_files_to_compile(entry_point, "*.cpp", false)      -- *.c files under entry_point_dir (recursively)
  local output_dir = utils.os_path(vim.fn.getcwd() .. "/bin/")               -- working_directory/bin/
  local output = utils.os_path(vim.fn.getcwd() .. "/bin/program")            -- working_directory/bin/program
  local handle = io.popen("pkg-config --cflags --libs sdl3")
-- Change this line:

-- To this:
local cflags_handle = io.popen("pkg-config --cflags sdl3")
local cflags = cflags_handle:read("*a"):gsub("\n","")
cflags_handle:close()

local libs_handle = io.popen("pkg-config --libs sdl3")
local libs = libs_handle:read("*a"):gsub("\n","")
libs_handle:close()

local arguments = "-Wall -g " .. cflags
      ...
      strategy = { "orchestrator",
        tasks = {{ name = "- Build & run program → \"" .. entry_point .. "\"",
          cmd = "rm -f \"" .. output .. "\" || true" ..                           -- clean
              ...
              " && g++ " .. arguments .. " " .. files .. " -o \"" .. output .. "\" " .. libs ..
              " && \"" .. output .. "\"" ..                                       -- run
              " && echo \"\n" .. entry_point .. "\"" ..                             -- echo
              " && echo \"" .. final_message .. "\"",
